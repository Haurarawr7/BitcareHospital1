...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action == 'insert') {
        ...
        $query = "INSERT INTO rekam_medis (no_rekam_medis, tanggal_rekam, id_pasien, riwayat, id_dokter) 
            VALUES ('$tanggal_rekam', '$id_pasien', '$riwayat', '$id_dokter')";
        mysqli_query($koneksi, $query);
    }
    elseif ($action == 'edit') {
        $no_rekam_medis = $_POST["no_rekam_medis"];
        $no_rekam_medis = $_POST["oldno_rekam_medis"];
        $tanggal_rekam = $_POST["tanggal_rekam"];
        $id_pasien = $_POST["id_pasien"];
        $riwayat = $_POST["riwayat"];
        $id_dokter = $_POST["id_dokter"];
        
        $query = "UPDATE rekam_medis SET no_rekam_medis='$no_rekam_medis', tanggal_rekam='$tanggal_rekam', id_pasien='$id_pasien', riwayat='$riwayat', id_dokter='$id_dokter' WHERE no_rekam_medis='$oldno_rekam_medis'";
        mysqli_query($koneksi, $query);
...
<body>
    ...
    <div id="rekamMedisModal" class="formulir">
        <div class="formulir-content">
            ...
            <form id="rekamMedisForm" method="POST">
                ...
                <input type="hidden" name="oldno_rekam_medis" id="oldnoRekamMedisInput" value="">

                <div class="form-group">
                    <label for="noRekamMedis">No. Rekam Medis:</label>
                    <input type="text" id="noRekamMedis" name="no_rekam_medis" required>
                </div>
                <div class="form-group">
    ...
    <script>
        function openModal(rekamData = null) {
            document.getElementById('rekamMedisForm').reset();
            document.getElementById('oldnoRekamMedisInput').value = '';
            ...
            if (rekamData) {
                ...
                document.getElementById('noRekamMedis').value = rekamData.no_rekam_medis;
                document.getElementById('oldnoRekamMedisInput').value = rekamData.no_rekam_medis;
                document.getElementById('tanggalRekamMedis').value = rekamData.tanggal_rekam;
                document.getElementById('idPasienRm').value = rekamData.id_pasien;
                document.getElementById('riwayatRekamMedis').value = rekamData.riwayat;
                document.getElementById('idDokterRm').value = rekamData.id_dokter;
            } else {
        ...
        function openDeleteModal(no_rekam_medis) {
            document.getElementById('no_rekam_medis_delete').value = no_rekam_medis;
        ...
        function toggleEditForm(no_rekam_medis, tanggal_rekam, id_pasien, riwayat, id_dokter) {
            openModal({
